-- Fix: ac_total_flight_mins and ajlb_row_number window functions used default RANGE
-- frame with ORDER BY off_block_time_epoch. When two NEW flights share the same
-- off_block_time_epoch, RANGE treats them as one "group" and both receive the same
-- cumulative total and the same logbook row position.
-- Fix: add l.flight_id as a tiebreaker (it is unique) and use ROWS frame so each
-- row is counted individually. Matches the existing ROWS fix already on ac_total_landings.
