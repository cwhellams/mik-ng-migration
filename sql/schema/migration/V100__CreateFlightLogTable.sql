CREATE TABLE flight.logs (
    ...
    number_of_night_landings SMALLINT NOT NULL DEFAULT 0,
    ...
    non_billing_approved_by_member_id VARCHAR(9) REFERENCES member.register (member_id),
    priv_or_com_flight CHAR(1) NOT NULL CHECK (priv_or_com_flight IN ('P', 'C')),
    ...
    FOREIGN KEY (aircraft_registration) REFERENCES flight.aircraft (registration),
    ...
    CONSTRAINT check_flight_time_sequence CHECK (
    ...
    CONSTRAINT check_fuel_oil_pob_ldg_reasonable CHECK (
        fuel_remaining_litres >= 0
        AND persons_on_board >= 1
        AND number_of_landings >= 1
    ),
    CONSTRAINT check_epochs_not_future CHECK (
        on_block_time_epoch <= EXTRACT(
            EPOCH
            FROM NOW()
        )
        AND off_block_time_epoch <= EXTRACT(
            EPOCH
            FROM NOW()
        )
        AND takeoff_time_epoch <= EXTRACT(
            EPOCH
            FROM NOW()
        )
        AND landing_time_epoch <= EXTRACT(
            EPOCH
            FROM NOW()
        )
    ),
    ...
    takeoff_time_utc TIMESTAMPTZ NOT NULL GENERATED ALWAYS AS (TO_TIMESTAMP(takeoff_time_epoch)) STORED,
    landing_time_utc TIMESTAMPTZ NOT NULL GENERATED ALWAYS AS (TO_TIMESTAMP(landing_time_epoch)) STORED,
    on_block_time_utc TIMESTAMPTZ NOT NULL GENERATED ALWAYS AS (TO_TIMESTAMP(on_block_time_epoch)) STORED,
    block_mins INTEGER NOT NULL GENERATED ALWAYS AS (
        (on_block_time_epoch - off_block_time_epoch) / 60
    ) STORED,
    flight_mins INTEGER NOT NULL GENERATED ALWAYS AS (
        (landing_time_epoch - takeoff_time_epoch) / 60
    ) STORED,
    block_time TEXT NOT NULL GENERATED ALWAYS AS (
        EPOCH_DIFF_TO_HHMM(off_block_time_epoch, on_block_time_epoch)
    ) STORED,
    flight_time TEXT NOT NULL GENERATED ALWAYS AS (
