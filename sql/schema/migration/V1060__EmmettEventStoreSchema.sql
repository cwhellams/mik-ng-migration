-- Emmett event store schema for version 0.42.x
-- Source: @event-driven-io/emmett-postgresql v0.42.1 (schema_0_42_0 snapshot)
-- All instructor qualification events are stored in the 'emt:default' partition.
-- autoMigration is set to 'None' on the event store — Flyway owns this schema.
...
IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'emt_append_to_stream') THEN
    ...
    BEGIN
        ...
        ELSE
            ...
            WHERE stream_id = v_stream_id
              AND stream_position = v_expected_stream_position
              AND partition = v_partition
              AND is_archived = FALSE;
        ...
        all_messages_insert AS (
            INSERT INTO emt_messages
                (stream_id, stream_position, partition, message_data, message_metadata,
                 message_schema_version, message_type, message_kind, message_id, transaction_id)
            SELECT
                v_stream_id, ev.stream_position, v_partition, ev.message_data, ev.message_metadata,
                ev.schema_version, ev.message_type, ev.message_kind, ev.message_id, v_transaction_id
            FROM ev
            RETURNING global_position
        )
        SELECT array_agg(global_position ORDER BY global_position) INTO v_global_positions
        FROM all_messages_insert;
...
-- Create the default partition used by all event streams
...
IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'store_processor_checkpoint') THEN
    ...
    BEGIN
        IF p_check_position IS NOT NULL THEN
            ...
            SET "last_processed_checkpoint"     = p_position,
            ...
            IF current_position = p_position THEN RETURN 0;
            ELSIF current_position > p_position THEN RETURN 3;
            ELSE RETURN 2;
            END IF;
        END IF;
        ...
        BEGIN
            INSERT INTO "emt_processors"(
                "processor_id", "version", "last_processed_checkpoint",
                "partition", "last_processed_transaction_id", "created_at", "last_updated"
            )
...
BEGIN
    ...
    ownership_check AS (
        INSERT INTO emt_processors (
            processor_id, partition, version, processor_instance_id,
            status, last_processed_checkpoint, last_processed_transaction_id,
            created_at, last_updated
        )
        SELECT p_processor_id, p_partition, p_version, p_processor_instance_id,
               'running', '0000000000000000000', '0'::xid8, now(), now()
    ...
    projection_status AS (
        INSERT INTO emt_projections (name, partition, version, type, kind, status, definition)
        SELECT p_projection_name, p_partition, p_version, p_projection_type,
               p_projection_kind, 'async_processing', '{}'::jsonb
...
BEGIN
    ...
    upsert_result AS (
        INSERT INTO emt_projections (name, partition, version, type, kind, status, definition, created_at, last_updated)
...
-- Grant access for the application database user
GRANT SELECT, INSERT, UPDATE ON emt_streams TO ${app_db_user};
GRANT SELECT, INSERT ON emt_messages TO ${app_db_user};
GRANT SELECT, INSERT, UPDATE ON emt_processors TO ${app_db_user};
GRANT SELECT, INSERT, UPDATE ON emt_projections TO ${app_db_user};
GRANT USAGE, SELECT ON SEQUENCE emt_global_message_position TO ${app_db_user};

-- Grant on the default partition tables (created by emt_add_partition above)
GRANT SELECT, INSERT, UPDATE ON emt_streams_emt_default TO ${app_db_user};
GRANT SELECT, INSERT, UPDATE ON emt_streams_emt_default_active TO ${app_db_user};
GRANT SELECT, INSERT, UPDATE ON emt_streams_emt_default_archived TO ${app_db_user};
GRANT SELECT, INSERT ON emt_messages_emt_default TO ${app_db_user};
GRANT SELECT, INSERT ON emt_messages_emt_default_active TO ${app_db_user};
GRANT SELECT, INSERT ON emt_messages_emt_default_archived TO ${app_db_user};
GRANT SELECT, INSERT, UPDATE ON emt_processors_emt_default TO ${app_db_user};
GRANT SELECT, INSERT, UPDATE ON emt_projections_emt_default TO ${app_db_user};
