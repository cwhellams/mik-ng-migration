...
/**
 * Express serialises a Buffer as `{ type: 'Buffer', data: number[] }`, so the
 * value arrives in one of two shapes depending on how it was sent. It was typed
 * `any` in `apps/frontend`; this package is held to zero ESLint errors, so the
 * two shapes are named instead.
 */
type SerialisedBuffer = { type: 'Buffer'; data: number[] }

const isSerialisedBuffer = (
  value: SerialisedBuffer | ArrayLike<number>,
): value is SerialisedBuffer => (value as SerialisedBuffer).type === 'Buffer'

const bufferToDataUrl = (buf: SerialisedBuffer | ArrayLike<number> | null | undefined) => {
  if (!buf) return null
  const bytes = isSerialisedBuffer(buf) ? buf.data : Array.from(buf)
  return `data:image/png;base64,${btoa(String.fromCharCode(...bytes))}`
}

const DocumentsPage = ({ canManage = false }: { canManage?: boolean }) => {
  const { t } = useTranslation()
  const { showSnackbar } = useSnackbar()
  // Supplied by the app, not derived here: `apps/frontend` renders this page
  // read-only and `apps/admin` renders it with the DOCUMENT_ADMIN check (#1233).
  // Passing the capability in is what actually removes document management from
  // the member UI — the member app has no value it can pass that turns it on.
  const isDocumentAdmin = canManage
