// eslint-disable-next-line @typescript-eslint/triple-slash-reference -- an `import` cannot pull in an ambient module declaration; see the note below.
/// <reference path="../types/country-telephone-data.d.ts" />
// The reference is load-bearing: `country-telephone-data` ships no types, and an
// ambient `declare module` in this package is not picked up by a consuming app's
// program (whose `include` covers only its own `src`). Pointing at it from the one
// file that imports the module pulls it into every program that compiles this file.
