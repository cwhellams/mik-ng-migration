...
import { AuditableSchema, BooleanSchema, OptionalLimitOffsetSchema } from './schema.ts'
...
export const DownloadDocumentSchema = z.object({
  ...
  // Uint8Array rather than Buffer: the handler still returns a Node Buffer (which
  // is a Uint8Array), but naming Buffer here would evaluate an undefined global in
  // the browser, and this module is imported by the frontend. Over the wire Express
  // serialises it as { type: 'Buffer', data: number[] } either way.
  qrCode: z.custom<Uint8Array>((value) => value instanceof Uint8Array),
})
...
export const DocumentIdSchema = z.string().transform((val, ctx) => {
