...
router.post('/request-otp', async (req: Request<{}, {}, OTPRequestBody, Record<string, any>>, res: Response) => {
...
router.post('/verify-otp', async (req: Request<{}, {}, OTPVerifyBody>, res: Response) => {
...
router.post('/refresh-token', async (req: Request<{}, {}, {}>, res: Response) => {
    try {
        ...
        if (!refreshToken) {
            return res.status(401).send({ code: 'refresh_failed',error: 'Refresh token not found' });
        }
        ...
        if (!user) {
            return res.status(401).send({ code: 'refresh_failed', error: 'User not found' });
        }

        const storedToken: StoredToken | undefined = undefined; // TODO: find token by userId and sessionId

        if (!storedToken) {
            return res.status(401).send({ code: 'refresh_failed', error: 'Invalid session or token not found' });
        }

        const isValid = await bcrypt.compare(refreshToken, storedToken.token);

        if (!isValid) {
            return res.status(401).send({ code: 'refresh_failed', error: 'Invalid refresh token' });
        }

        if (storedToken.expiresAt < new Date()) {
            // TODO: delete the old token from db
            return res.status(401).send({ code: 'refresh_failed', error: 'Refresh token has expired' });
        }
