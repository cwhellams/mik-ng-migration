...
import { LineItemsTable } from '@mik/ui/components/expenseShared'
import type { EditableLineItem } from '@mik/ui/utils/expenseHelpers'
...
export function ExpenseClaimAdminDetail() {
  ...
  // `sudo && hasAccess(...)` in the member app, which is what hasSudoAccess is.
  // Here that resolves to a plain permission check, since this app is always in
  // admin context.
  const { hasSudoAccess } = useRoles()
  const canRevealHetu = hasSudoAccess(MIKPermissions.EXPENSE_HETU_ADMIN)
