...
import { ADMIN_MEMBER_ID, AIRCRAFT_REGISTRATION, FIXTURE_TIMESTAMP, MEMBER_ID } from '../fixtures'

/**
 * Liquid Management System fixtures (#1119), ported for `apps/admin`.
 *
 * Mirrors `apps/frontend/src/test/fixtures/liquid.ts` — same cast, same
 * aircraft, same shapes — but sourced from this app's own `../fixtures`
 * re-export rather than `@mik/ui/test/fixtures/cast` directly, matching how
 * every other admin fixture file in this directory imports the cast
 * constants. A builder returns a complete, valid entity so a test names only
