...
export const getProductProperties = (
  ...
  api
    .get<ProductProperty[]>(`products/${productId}/properties`)
    .then((r) => r.data)
...
export const upsertProductProperty = (
  ...
  api
    .put<ProductProperty[]>(`products/${productId}/properties`, data)
    .then((r) => r.data)

export const deleteProductProperty = (
  productId: string,
  propertyId: number
): Promise<void> => api.delete(`products/${productId}/properties/${propertyId}`)
...
export const updateDiscountCode = (
  ...
  api.put<DiscountCode>(`discount-codes/${id}`, data).then((r) => r.data)
...
export const getPrepaidPackages = (
  aircraft?: string
...
export const createPrepaidPackage = (
...
export const getMemberPackages = (
  ...
  prepaidApi
    .get<
      MemberPackage[]
    >('member-packages', { params: memberId ? { memberId } : {} })
    .then((r) => r.data)
...
export const extendExpiry = (data: {
  ...
  prepaidApi
    .post<{ updated: number }>('extend-expiry', data)
    .then((r) => r.data)

// ── Aircraft ──────────────────────────────────────────────────────────────────
export const getActiveAircraft = (): Promise<AircraftListResponse> =>
  aircraftApi
    .get<AircraftListResponse>('', { params: { activeOnly: true } })
    .then((r) => r.data)
