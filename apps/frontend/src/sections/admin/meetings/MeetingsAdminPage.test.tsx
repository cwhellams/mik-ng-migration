...
import { aMemberListResponse } from '../../../test/fixtures'
...
/** Signs in as a meeting admin and renders with sudo on, which the page requires. */
const renderAsMeetingAdmin = () => {
  signInWithPermissions(MIKPermissions.MEETING_ADMIN)
  return renderWithProviders(<MeetingsAdminPage />, { sudo: true })
}
...
describe('MeetingsAdminPage access', () => {
  it('refuses a member without the meeting-admin permission', async () => {
    ...
    renderWithProviders(<MeetingsAdminPage />, { sudo: true })

    expect(await screen.findByRole('alert')).toHaveTextContent(/access/i)
  })

  it('refuses a meeting admin who has not switched sudo on', async () => {
    meetingsApi()
    signInWithPermissions(MIKPermissions.MEETING_ADMIN)

    renderWithProviders(<MeetingsAdminPage />, { sudo: false })

    expect(await screen.findByRole('alert')).toHaveTextContent(/access/i)
  })

  it('lets a meeting admin in with sudo on', async () => {
    meetingsApi()
