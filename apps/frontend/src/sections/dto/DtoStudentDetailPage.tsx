...
function flightStatus(
  ...
  const flightAttempts = attempts.filter(
    (a) => a.syllabusFlightId === flight.flightId
  )
  if (
    flightAttempts.some(
      (a) => a.verificationResult === 'APPROVED' && a.requiresReverification
    )
  )
    return 'needs_reverification'
  if (flightAttempts.some((a) => a.verificationResult === 'APPROVED'))
    return 'approved'
  if (flightAttempts.some((a) => a.verificationResult === 'FAILED'))
    return 'failed'
  if (flightAttempts.some((a) => a.verificationResult === null))
    return 'pending'
  return 'not_flown'
}
...
export default function DtoStudentDetailPage() {
  ...
  const { data, isLoading, error } = useApi<StudentProgressDetail>({
    url: memberSyllabusId
      ? `v1/dto/member-syllabus/${memberSyllabusId}/detail`
      : undefined,
    skipFetch: !memberSyllabusId,
  })

  const flights: SyllabusFlight[] =
    data?.memberSyllabus?.syllabusDetail?.flights ?? []
  ...
  return (
    <Box>
      ...
      <Title
        label={data?.memberSyllabus?.memberName ?? t('dto.detail.title')}
      />
              ...
              <Stack spacing={2}>
                {flights.map((flight) => {
                  ...
                  const approvedAttempt = attempts.find(
                    (a) =>
                      a.syllabusFlightId === flight.flightId &&
                      a.verificationResult === 'APPROVED'
                  )
                  ...
                  return (
                    ...
                    >
                      ...
                      {displayAttempt &&
                        displayAttempt.itemOutcomes.length > 0 && (
                          <>
                            <Divider sx={{ my: 1 }} />
                            <Table size='small'>
                              <TableHead>
                                <TableRow>
                                  ...
                                  <TableCell>
                                    {t('dto.detail.outcome')}
                                  </TableCell>
                                </TableRow>
                              </TableHead>
                              <TableBody>
                                {displayAttempt.itemOutcomes.map((o) => {
                                  const item = flight.items?.find(
                                    (i) => i.itemId === o.itemId
                                  )
                                  return (
                      ...
                      {displayAttempt?.verifiedAt && (
                        ...
                        >
                          {t('dto.detail.verifiedBy', {
                            name:
                              displayAttempt.verifierName ??
                              displayAttempt.verifiedBy,
                        ...
                        >
                          {displayAttempt.flightDate && (
                            <Typography variant='body2' color='text.secondary'>
                              {t('dto.detail.date')}:{' '}
                              <strong>{displayAttempt.flightDate}</strong>
                            </Typography>
                          )}
